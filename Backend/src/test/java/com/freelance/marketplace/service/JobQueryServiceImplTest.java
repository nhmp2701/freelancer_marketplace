...
class JobQueryServiceImplTest {
    ...
    private final JobQueryServiceImpl service = new JobQueryServiceImpl(
            mock(JobRepository.class), mock(UserRepository.class), mock(JobMapper.class));

    @Test
    void rejectsInvertedBudgetRange() {
        assertThatThrownBy(() -> service.searchJobs(null, BigDecimal.TEN, BigDecimal.ONE,
    ...
    void rejectsUnknownSortField() {
        assertThatThrownBy(() -> service.searchJobs(null, null, null,
                null, null, 0, 20, "client.passwordHash", "desc"))
                .isInstanceOf(IllegalArgumentException.class);
    }
}
