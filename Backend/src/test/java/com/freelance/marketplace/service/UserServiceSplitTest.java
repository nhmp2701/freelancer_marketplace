...
class UserServiceSplitTest {
  ...
  void updatesOnlyProvidedProfileFields() {
    ...
    UserProfileResponse response =
        new UserProfileServiceImpl(userRepository).updateUserProfile(user.getEmail(), request);
  ...
  void addsAndRemovesAnExistingSkill() {
    ...
    User user = User.builder().email("user@example.com").skills(new HashSet<>()).build();
    ...
    UserSkillServiceImpl service =
        new UserSkillServiceImpl(userRepository, new SkillResolver(skillRepository));
